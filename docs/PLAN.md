...
4. Analyze accessible evidence conservatively. Separate findings, confidence and coverage limitations. Missing data never means secure.
...
| Networking | `network` | Addresses, links, routing/policy rules, sockets, Wi-Fi/Ethernet/tethering, firewall tables | Root for firewall; namespace/offload/eBPF limits; projection role needs OEM evidence |
| Networking | `passive_network` | Bounded `tcpdump -i any` PCAP, tshark cleartext protocol candidates | Device tcpdump/timeout, permissions and host tshark; no decryption |
