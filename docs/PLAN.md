...
| Networking | `radamsa_fuzz` | Separate, explicit TCP/UDP Radamsa campaigns with seed files, bounded payloads, durable timestamped journals and response metadata | Requires manual target selection and Radamsa; sending payloads can alter state or crash services; health/crash correlation remains external |
...
| Wireless | `bluetooth` | Local inventory; optional host MAC-targeted SDP, BLE services/properties/descriptors, BLE reads, advertised RFCOMM/L2CAP connection probes and bounded target-scoped BLE write probes/fuzzing | Host adapter/tools required; existing bonds affect results; writes can change state and do not prove unauthenticated access |
