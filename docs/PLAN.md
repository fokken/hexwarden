...
| Wireless | `bluetooth` | Local inventory; optional Android built-in HCI snoop capture; optional host MAC-targeted SDP, BLE services/properties/descriptors, BLE reads and notification subscriptions, advertised RFCOMM/L2CAP connection probes and bounded target-scoped Classic/BLE payload probes | HCI logging must be enabled in Developer Options and protected log paths may require root; host adapter/tools required; existing bonds affect results; payloads/subscriptions can change state or expose data and do not prove unauthenticated access |
