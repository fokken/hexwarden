...
| Module | Collected information | Automated checks and findings | Limits |
|---|---|---|---|
| `bluetooth` | Android Bluetooth manager/service/sysfs inventory; optional host adapter state, Classic SDP services, RFCOMM/L2CAP endpoints, BLE services/characteristics/descriptors/properties | `HW-BT-001` advertised Classic services; `HW-BT-002` successful Classic endpoint connections; `HW-BT-003` advertised BLE write support; `HW-BT-004` readable BLE characteristics | Host MAC is user-supplied and not tied to ADB; existing bonds affect results; no characteristic writes, notifications or application payloads are sent |
