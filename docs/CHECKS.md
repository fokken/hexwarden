...
| `radamsa_fuzz` | User-selected TCP/UDP fuzz campaign, seed files, generated payloads, timestamps, hashes, send outcomes and bounded responses | Campaign availability, target/seed selection and completion checks | Separate opt-in flag; never selects listeners automatically; requires Radamsa and authorized targets; send/response behavior is not a vulnerability verdict |
